...
[Documentation](docs/) · [AI-agent docs](docs/for_ai/index.en.md) · [Integration guide](docs/integration-guide.en.md)
...
An optional post-upload byte cache — `media_storage=`, see the [integration guide](docs/integration-guide.en.md).
...
The full error table, DI, fakes for tests, `reconfigure()` for live token rotation — [`docs/integration-guide.en.md`](docs/integration-guide.en.md).
...
Every generated file carries `Generated by forge — DO NOT EDIT`: the spec doesn't always match what the API actually returns, and a regen would wipe a hand patch. What's been verified live, what's known-broken, and how to patch a spec/reality mismatch — [`docs/codegen-runbook.en.md`](docs/codegen-runbook.en.md).
...
Releases ride a `v*` tag — build, gate and PyPI publish are automatic; the maintainer procedure lives in [CONTRIBUTING.en.md](CONTRIBUTING.en.md).
