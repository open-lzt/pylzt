...
```bash
pip install pylzt
# unreleased main:  pip install "git+https://github.com/open-lzt/pylzt.git"
```
...
`python -m dev.codegen build` — don't hand-edit a file with the auto-gen header.
A handful of models still carry a live-verification note in their docstring where
the spec's declared shape doesn't match a real response — everything else has
been checked against production traffic.
