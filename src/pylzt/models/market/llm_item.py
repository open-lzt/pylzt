...
Fourth pass (category-unique fields, same date, biggest error volume in this batch —
334 errors): every `chatgpt_*` field plus `gptSubType` is present only on a subset
of listings (accounts without a linked ChatGPT subscription omit the whole group) —
all eight loosened to nullable.

Fifth pass (same date, after re-running the e2e suite against the fourth-pass
fixes): `emailLoginUrl` is also absent on a meaningful fraction of listings —
loosened to nullable (same shared-field pattern as instagram/minecraft/riot/roblox).
...
class LLMItem(BaseItem):
    ...
    chatgpt_item_id: int | None = None
    chatgpt_country: str | None = None
    chatgpt_register_date: int | None = None
    chatgpt_phone: int | None = None
    chatgpt_subscription: str | None = None
    chatgpt_subscription_ends: int | None = None
    chatgpt_subscription_auto_renew: int | None = None
