"""Generated by forge — DO NOT EDIT.
Sourced from the official readme.io OpenAPI spec; re-run `python -m dev.codegen build`
to update.
"""

from __future__ import annotations

from pylzt.enums.market import Action
from pylzt.methods.base import BaseMethod
from pylzt.models.market import (
    MarketStatusItemResponse,
    MarketStatusMessageResponse,
    PublishingCheckItem,
    PurchasingCheckResponse,
    PurchasingConfirmResponse,
)
from pylzt.types import HttpMethod


# Source: official OpenAPI spec, operationId "Purchasing.Check".
class PurchasingCheck(BaseMethod[PurchasingCheckResponse]):
    """Generated by forge — DO NOT EDIT. Check Account.
    Sourced from the official OpenAPI spec (readme.io).
    Checking account for validity.
    > ❗️ If you receive a "retry_request" error, you should repeat the same request (up to a maximum of 100 times).

    Args:
        item_id: Item id.

    Docs: https://lzt-market.readme.io/reference/purchasingcheck
    """

    __http_method__ = HttpMethod.POST
    __url__ = "/{item_id}/check-account"
    __returning__ = PurchasingCheckResponse

    item_id: int


# Source: official OpenAPI spec, operationId "Purchasing.Confirm".
class PurchasingConfirm(BaseMethod[PurchasingConfirmResponse]):
    """Generated by forge — DO NOT EDIT. Confirm Buy.
    Sourced from the official OpenAPI spec (readme.io).
    Confirm buy.
    > ❗️ This method doesn't check account for validity. If you want to confirm validity before buying, you should use [FastBuy](https://lzt-market.readme.io/reference/purchasingfastbuy) method

    Args:
        item_id: Item id.
        price: Current price of account in your currency.
        balance_id: Balance ID that will be used to purchase specified item.

    Docs: https://lzt-market.readme.io/reference/purchasingconfirm
    """

    __http_method__ = HttpMethod.POST
    __url__ = "/{item_id}/confirm-buy"
    __returning__ = PurchasingConfirmResponse

    item_id: int
    price: int | None = None
    balance_id: int | None = None


# Source: official OpenAPI spec, operationId "Purchasing.DiscountCancel".
class PurchasingDiscountCancel(BaseMethod[MarketStatusMessageResponse]):
    """Generated by forge — DO NOT EDIT. Cancel Discount Request.
    Sourced from the official OpenAPI spec (readme.io).
    Cancel a requested discount for the specified item.

    Args:
        item_id: Item id.

    Docs: https://lzt-market.readme.io/reference/purchasingdiscountcancel
    """

    __http_method__ = HttpMethod.DELETE
    __url__ = "/{item_id}/discount"
    __returning__ = MarketStatusMessageResponse

    item_id: int


# Source: official OpenAPI spec, operationId "Purchasing.DiscountRequest".
class PurchasingDiscountRequest(BaseMethod[MarketStatusMessageResponse]):
    """Generated by forge — DO NOT EDIT. Discount Request.
    Sourced from the official OpenAPI spec (readme.io).
    Request a discount for the specified item.

    Args:
        item_id: Item id.
        discount_price: Requested discounted price.
        message: Message to the seller.
        auto_buy: Automatically buy the item once the discount is accepted.
        balance_id: Balance ID that will be used to purchase specified item.

    Docs: https://lzt-market.readme.io/reference/purchasingdiscountrequest
    """

    __http_method__ = HttpMethod.POST
    __url__ = "/{item_id}/discount"
    __returning__ = MarketStatusMessageResponse

    item_id: int
    discount_price: float
    message: str | None = None
    auto_buy: bool | None = None
    balance_id: int | None = None


# Source: official OpenAPI spec, operationId "Purchasing.DiscountReview".
class PurchasingDiscountReview(BaseMethod[MarketStatusMessageResponse]):
    """Generated by forge — DO NOT EDIT. Approve/Decline Discount Request.
    Sourced from the official OpenAPI spec (readme.io).
    Approve/Decline a discount request.

    Args:
        item_id: Item id.
        user_id: User ID.
        action: Approve/Reject requested discount.
        discount_price: Offer a different discount for an amount other than what the user requested. Uses your default currency.
        message: This message will be shown in buyer discount review notification.

    Docs: https://lzt-market.readme.io/reference/purchasingdiscountreview
    """

    __http_method__ = HttpMethod.PUT
    __url__ = "/{item_id}/discount"
    __returning__ = MarketStatusMessageResponse

    item_id: int
    user_id: int
    action: Action | None = None
    discount_price: float | None = None
    message: str | None = None


# Source: official OpenAPI spec, operationId "Purchasing.FastBuy".
class PurchasingFastBuy(BaseMethod[MarketStatusItemResponse[PublishingCheckItem]]):
    """Generated by forge — DO NOT EDIT. Fast Buy Account.
    Sourced from the official OpenAPI spec (readme.io).
    Check and buy account.
    > ❗️ If you receive a "retry_request" error, you should repeat the same request (up to a maximum of 100 times).

    Args:
        item_id: Item id.
        price: Current price of account in your currency.
        balance_id: Balance ID that will be used to purchase specified item.

    Docs: https://lzt-market.readme.io/reference/purchasingfastbuy
    """

    __http_method__ = HttpMethod.POST
    __url__ = "/{item_id}/fast-buy"
    __returning__ = MarketStatusItemResponse[PublishingCheckItem]

    item_id: int
    price: float | None = None
    balance_id: int | None = None
